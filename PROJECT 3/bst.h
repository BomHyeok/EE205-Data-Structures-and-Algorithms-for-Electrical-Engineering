...
class IntBST {
 public:
	...
	void printall();
	void printrecur(IntEntry* E);

	protected:
	int cnt;           
	IntBinaryTree T;
	int findRecursion(int k, IntEntry* E);
	int countRecursion(int k1, int k2, IntEntry* E, int count);
};
...
class IntRBTree : public IntBST {
 public:
	virtual void insert(int k);
	virtual bool remove(int k);
	void restructuring(int m, IntEntry* E, IntEntry *PE, IntEntry *GPE);
	void recoloring(IntEntry *E, IntEntry *PE, IntEntry *GPE, IntEntry *UE);
};
